...
#These pin numbers represent what gpio pins they take up
TARE_PIN = 17
MOON_PIN = 12
SUN_PIN = 26
MARS_PIN = 25
...
DISPLAY_MS = 5000 #planet buttons selection shows for seconds

# Put your calibration values here:
OFFSET = 258980        # <-- replace with your saved offset (int)
SCALE_FACTOR = 202.22    # <-- replace with your saved scale_factor (float)

READ_SAMPLES = 10
UPDATE_MS = 200

BACKGROUND_IMAGE = "Earth.jpg"  # image file in the SAME folder
WINDOW_W = 800
WINDOW_H = 480
# ---------------------------------------
...
# ----- UI labels (placed on top of background) -----
...
def set_background(which=None):
    """which=None restores base background: otherwise uses planet background"""
    global bg_reset_job

    #cancel previously scheduled reset, so rapid presses behave nicely
...
def tare():
	global OFFSET
	status_lbl.config(text="Reseting scale...")
	root.update()
...
def update():
    ...
    if grams is None:
        ...
        if show_space:
            space_lbl.config(text=f"{int(grams * 0)} g on the Space")
